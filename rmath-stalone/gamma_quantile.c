 ...
 *	Algorithm AS 91 ("ppchi2") and via gamma_cdf(.) AS 239.
 *
 *	R core improvements:
 *	o  lower_tail, log_p
...
#ifdef DEBUG_gamma_quantile
# define DEBUG_q
#endif

double chisq_quantile_appr(double p, double nu, double g /* = log Gamma(nu/2) */,
...
#define C10	13.32
	...
	if(nu < (-1.24)*(p1 = R_DT_log(p))) {	/* for small chi-squared */
		...
		x = normal_quantile(p, 0, 1, lower_tail, log_p);
		p1 = 2./(9*nu);
...
double gamma_quantile(double p, double alpha, double scale, int lower_tail, int log_p)
/*			shape = alpha */
{
#define EPS1 1e-2
...
#ifdef _DO_WARN_gamma_quantile_
...
#ifdef DEBUG_gamma_quantile
	REprintf("gamma_quantile(p=%7g, alpha=%7g, scale=%7g, l.t.=%2d, log_p=%2d): ",
		 p,alpha,scale, lower_tail, log_p);
#endif
	g = lgamma(alpha);/* log Gamma(v/2) */

	/*----- Phase I : Starting Approximation */
	ch = chisq_quantile_appr(p, /* nu= 'df' =  */ 2*alpha, /* lgamma(nu/2)= */ g,
...
/*----- Phase II: Iteration
 *	Call gamma_cdf() [AS 239]	and calculate seven term taylor series
 */
	c = alpha-1;
	...
	for(i=1; i <= MAXIT; i++ ) {
		q = ch;
		p1 = 0.5*ch;
		p2 = p_ - gamma_cdf_raw(p1, alpha, /*lower_tail*/TRUE, /*log_p*/FALSE);
#ifdef DEBUG_gamma_quantile
...
#ifdef DEBUG_q
	MATHLIB_WARNING3("gamma_quantile(%g) not converged in %d iterations; rel.ch=%g\n",
			 p, MAXIT, ch/fabs(q - ch));
#endif
/* was
 *    ML_ERROR(ME_PRECISION, "gamma_quantile");
 * does nothing in R !*/

END:
/* PR# 2214 :	 From: Morten Welinder <terra@diku.dk>, Fri, 25 Oct 2002 16:50
   --------	 To: R-bugs@biostat.ku.dk     Subject: gamma_quantile precision
   ...
   */
	...
	if(max_it_Newton) {
		...
		if(x == 0) {
			...
			x = DBL_MIN;
			p_ = gamma_cdf(x, alpha, scale, lower_tail, log_p);
		...
		for(i = 1; i <= max_it_Newton; i++) {
			...
			if(fabs(p1) < fabs(EPS_N * p))
				break;
			/* else */
			if((g = gamma_pdf(x, alpha, scale, log_p)) == R_D__0) {
#ifdef DEBUG_q
			...
			p_ = gamma_cdf (t, alpha, scale, lower_tail, log_p);
