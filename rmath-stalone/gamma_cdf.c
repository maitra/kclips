 ...
 *	#include <Rmath.h>
 *
 *	double gamma_cdf (double x, double alph, double scale,
 ...
 * (cd ~/R/D/r-devel/Linux-inst/src/nmath; gcc -std=gnu99 -I. -I../../src/include -I../../../R/src/include -I/usr/local/include -DDEBUG_p -g -O2 -c ../../../R/src/nmath/gamma_cdf.c -o gamma_cdf.o)
 */
...
/* poisson_pdf_wrap (x_P_1,  lambda, g_log) ==
 *   poisson_pdf (x_P_1 - 1, lambda, g_log) :=  exp(-L)  L^k / gamma(k+1) ,  k := x_P_1 - 1
*/
static double
poisson_pdf_wrap (double x_plus_1, double lambda, int give_log)
{
#ifdef DEBUG_p
    REprintf (" poisson_pdf_wrap(x+1=%.14g, lambda=%.14g, log=%d)\n",
	      x_plus_1, lambda, give_log);
#endif
    if (!isfinite(lambda))
	return R_D__0;
    if (x_plus_1 > 1)
	return poisson_pdf_raw (x_plus_1 - 1, lambda, give_log);
    if (lambda > fabs(x_plus_1 - 1) * M_cutoff)
	return R_D_exp(-lambda - lgamma(x_plus_1));
    else {
	double d = poisson_pdf_raw (x_plus_1, lambda, give_log);
#ifdef DEBUG_p
	REprintf ("  -> d=poisson_pdf_raw(..)=%.14g\n", d);
#endif
	return give_log
 ...
 */
static double
gamma_cdf_smallx (double x, double alph, int lower_tail, int log_p)
{
	...
	double f2;
	if (alph > 1) {
	    f2 = poisson_pdf_raw (alph, x, log_p);
...
#define max_it 200000
    ...
    f0 = y/d;
    /* Needed, e.g. for  gamma_cdf(10^c(100,295), shape= 1.1, log=TRUE): */
	...
	if (b2 != 0) {
	    ...
	    if (fabs (f - of) <= DBL_EPSILON * MAX(f0, fabs(f))) {
#ifdef DEBUG_p
		REprintf(" %g iter.\n", i);
#endif
		return f;
    ...
    printf("Warning! ** NON-convergence in gamma_cdf()'s pd_lower_cf() f= %g.\n",
		    f);
...
pd_lower_series (double lambda, double y)
{
	 ...
	 *	  and is unnecessary e.g. for gamma_cdf(4e12, 121.1) */
...
dpnorm (double x, int lower_tail, double lp)
{
	...
	double d = normal_pdf (x, 0., 1., FALSE);
	return d / exp (lp);
    }
}
...
ppois_asymp (double x, double lambda, int lower_tail, int log_p)
{
    ...
    */
    ...
    np = normal_cdf (s2pt, 0.0, 1.0, !lower_tail, log_p);
	...
	double nd = normal_pdf (s2pt, 0., 1., log_p);
...
/* Here, assume that  (x,alph) are not NA  &  alph > 0 . */
...
#ifdef DEBUG_p
    REprintf("gamma_cdf_raw(x=%.14g, alph=%.14g, low=%d, log=%d)\n",
	     x, alph, lower_tail, log_p);
#endif
    R_P_bounds_01(x, 0., ML_POSINF);

    if (x < 1) {
	res = gamma_cdf_smallx (x, alph, lower_tail, log_p);
    } else if (x <= alph - 1 && x < 0.8 * (alph + 50)) {
	/* incl. large alph compared to x */
	double sum = pd_upper_series (x, alph, log_p);/* = x/alph + o(x/alph) */
	double d = poisson_pdf_wrap (alph, x, log_p);
#ifdef DEBUG_p
	REprintf(" alph 'large': sum=pd_upper*()= %.12g, d=poisson_pdf_w(*)= %.12g\n",
		 sum, d);
#endif
	if (!lower_tail)
	    res = log_p
	...
	double d = poisson_pdf_wrap (alph, x, log_p);
#ifdef DEBUG_p
	REprintf(" x 'large': d=poisson_pdf_w(*)= %.14g ", d);
#endif
	if (alph < 1) {
...
#endif
	return exp (gamma_cdf_raw (x, alph, lower_tail, 1));
    } else
	return res;
}
...
double gamma_cdf(double x, double alph, double scale, int lower_tail, int log_p)
{
	...
	return (x <= 0) ? R_DT_0: R_DT_1; /* <= assert  gamma_cdf(0,0) ==> 0 */
    return gamma_cdf_raw (x, alph, lower_tail, log_p);
}
/* From: terra@gnome.org (Morten Welinder)
 * To: R-bugs@biostat.ku.dk
 * Cc: maechler@stat.math.ethz.ch
 * Subject: Re: [Rd] gamma_cdf discontinuity (PR#7307)
 * Date: Tue, 11 Jan 2005 13:57:26 -0500 (EST)

 * this version of gamma_cdf appears to be quite good and certainly a vast
